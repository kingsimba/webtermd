...
**Shared capture-phase listener, not `attachCustomKeyEventHandler`:** The same `installClipboardShortcuts` helper is installed on every terminal host, including sudo prompts. The xterm.js API `attachCustomKeyEventHandler` fires during xterm.js's own keydown handling. Returning `false` tells xterm.js to skip sending the byte to onData, but the browser's native paste behavior on the hidden textarea can still fire independently, causing duplicate pastes. A capture-phase `addEventListener(..., true)` runs before any xterm.js internal handler and before the browser's default action. `stopImmediatePropagation()` kills the event entirely — xterm.js and the browser never see it. This matters more than it first appears: xterm.js's own `_keyDown` calls `event.preventDefault()` after sending the byte whenever `ctrlKey` is set, which suppresses the browser's native paste/copy default action. Stopping the event in capture phase is what lets the HTTP-fallback path below rely on the browser firing a native `paste` event.

**Two clipboard paths: secure context vs. plain HTTP:** `navigator.clipboard` is only defined in secure contexts (HTTPS or `localhost`). The server is an edge daemon frequently deployed behind a plain-HTTP reverse proxy on a remote host, where `navigator.clipboard` is `undefined` and any access to it throws synchronously — a throw the `.catch()` on a read cannot trap, because no promise is ever created. So the handler gates on `navigator.clipboard` existence and takes one of two paths:

- **Secure context (HTTPS / localhost):** Ctrl+V calls `navigator.clipboard.readText()` then `term.paste()`; Ctrl+C calls `navigator.clipboard.writeText(selection)`. Both call `e.preventDefault()` to suppress the native paste/copy on xterm's textarea.
- **Plain HTTP:** A hidden `<textarea>` (the "clipboard sink") is kept off-screen. For Ctrl+V, the handler focuses the sink; the browser fires a native `paste` event on it carrying `clipboardData`, which is read synchronously and forwarded to `term.paste()`. For Ctrl+C, the selection is written into the sink, the sink is selected, and `document.execCommand('copy')` runs — the only synchronous clipboard write available outside a secure context. The keydown satisfies the user-gesture requirement.

**Why `document.execCommand('copy')` for the HTTP copy path:** It is deprecated, but it remains the only synchronous clipboard write that works in non-secure contexts. `navigator.clipboard.writeText()` is unavailable there. The deprecation does not affect correctness here; it's a deliberate fallback, not the primary path.
